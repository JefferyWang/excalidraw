...
import { ChineseFontFaces } from "./ChineseFont";
...
export class Fonts {
  ...
  private static init() {
    ...
    // 注册自定义的字体
    for (const fontName in FONT_FAMILY) {
      const fontname = fontName as keyof typeof FONT_FAMILY;
      const fontid = FONT_FAMILY[fontname];
      if (fontid > 1000) {
        const font = FONT_METADATA[fontid];
        init(fontname, ...ChineseFontFaces);
      }
    }
